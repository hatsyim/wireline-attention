...
def _discrete_cmap_indexed(colors):
    ...
    """
    cmap = cplt.ListedColormap(colors, 'indexed')
    return cmap
...
class Logs:
    ...
    def visualize_crossplot(self, curve1, curve2, curvecolor=None,
                            ...
                            cmap='jet', cbar=True, cbarlabels=None, clims=None,
        ...
        if clims is None:
            clims = [np.nanmin(logcurvecolor), np.nanmax(logcurvecolor)]
        
        if grid:
            ax.grid()
        if curvecolor is not None:
            scatax = ax.scatter(logcurve1, logcurve2, c=logcurvecolor, s=2,
                                marker='o', edgecolors='none', alpha=0.7,
                                vmin=clims[0],
                                vmax=clims[1],
                                cmap=cmap)
        else:
            scatax = ax.scatter(logcurve1, logcurve2, marker='o', s=2,
        ...
        if cbar:
            ...
            if cbarlabels:
                scatax.set_clim(vmin=clims[0] - 0.5,
                                vmax=clims[1] + 0.5)
                cbar.set_ticks(np.arange(clims[0],
                                         clims[1]+1))
...
def confusion_matrix_facies(facies_true, facies_est, facies_labels, ax=None,
                            cmap='YlGnBu', figsize=(12, 10), title=None):
    """Visualize confusion matrix

     Parameters
    ----------
    facies_true : :obj:`np.ndarray`
        True facies
    facies_est : :obj:`np.ndarray`
        Estimated facies
    facies_labels : :obj:`list`, optional
        Facies labels
    ax : :obj:`plt.axes`
       Axes handle (if ``None`` make new figure)
    cmap : :obj:`str`, optional
        Colormap
    figsize : :obj:`tuple`, optional
             Size of figure
    ...
    """
    conf = confusion_matrix(facies_true, facies_est, labels=np.arange(len(facies_labels)),
                            normalize='pred')

    df_cm = pd.DataFrame(conf, index=list(facies_labels),
                         columns=list(facies_labels))
    ...
    heatmap = sns.heatmap(df_cm, annot=True, cmap=cmap, ax=ax)
    heatmap.yaxis.set_ticklabels(heatmap.yaxis.get_ticklabels(), rotation=0,
                                 ha='right', fontsize=10)
    heatmap.xaxis.set_ticklabels(heatmap.xaxis.get_ticklabels(), rotation=45,
                                 ha='right', fontsize=10)
    ax.set_ylabel('Predicted Facies')
    ax.set_xlabel('True Facies')
